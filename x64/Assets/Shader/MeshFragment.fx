...
void main()
{
	...
	if (!isLightEnable)
	{
		if (!isSelected)
			...
			daColor = vec4(diffColour.x + 0.5f, diffColour.y, diffColour.z + 0.5f, 1.0f);
		return;
	}
	...
	if (isSelected)
		daColor = vec4(outColour.x - 0.8f, outColour.y - 0.8f, outColour.z - 0.8f, 1.0f);
	else if (0 == iEaseType)
		daColor = vec4(outColour.x + 0.7f, outColour.y + 0.7f, outColour.z + 0.7f, 1.0f);
	else if (1 == iEaseType)
		daColor = vec4(outColour.x + 0.5f, outColour.y, outColour.z, 1.0f);
	else if (2 == iEaseType)
		daColor = vec4(outColour.x + 0.5f, outColour.y + 0.5f, outColour.z, 1.0f);
	else if (3 == iEaseType)
		daColor = vec4(outColour.x - 0.7f, outColour.y, outColour.z + 1.f, 1.0f);
	else
		daColor = vec4(outColour.x, outColour.y, outColour.z, 1.0f);
		

	//diffColour = theLights[0].diffuse;
	//daColor = vec4(diffColour.x, diffColour.y, diffColour.z, 1.0f);
